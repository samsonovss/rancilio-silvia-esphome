...
- живой статус фазы пролива с обратным отсчётом предсмачивания, паузы и основного пролива
- напоминание о промывке по количеству шотов и автоматическая программа backflush
- настраиваемая доза кофе и расчётный расход молотого кофе по завершённым шотам
...
### Статус пролива и расход кофе

Во время автоматического пролива `Silvia Brew Shot Status` показывает текущую фазу и обратный отсчёт:
...
`Silvia Coffee Dose Grams` хранит настроенную сухую дозу кофе на один шот. По умолчанию используется `14 г`, значение можно менять из Home Assistant. `Silvia Coffee Grounds Used` оценивает общий расход молотого кофе по общему счётчику шотов и настроенной дозе:

```text
Расход молотого кофе = Silvia Lifetime Shots × Silvia Coffee Dose Grams
```

Это оценка расхода сухого молотого кофе, а не вес напитка в чашке.
